...
> 本节记录测试代码已同步的新 API 合同覆盖项。当前工作区缺少 `.env.api-test`，尚未连接真实后端重跑 `pnpm test:api`，因此不改写上方 2026-03-16 的线上通过数据。

| 模块         | 新增/调整覆盖                                                                                                                                                                       |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Knowledge    | 覆盖非默认目录重命名/删除、文件批量移动校验、失败索引任务重试校验、文件 `indexing_task_id`、知识卡 `audit_reject_reason`、文件卡 `audit_status` / `audit_reject_reason` / `sources` |
| Knowledge QA | 搜索接口路径由 `POST /knowledge-qa/search` 更新为 `POST /knowledge-qa/{owner_type}/{owner_id}/search`                                                                               |
