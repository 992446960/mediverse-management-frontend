...
> 文档版本：v2.3  
> 更新日期：2026-03-23  
> 来源：飞书 Wiki（node `RjKPwTWUBivbaykfexbcBzaTnvb`）云文档 `D53xdwU3aoPGDnxAQLicIHEjn1d`，经 MCP `wiki_v2_space_getNode` + `docx_v1_document_rawContent` 同步  
...
### v2.3 (2026-03-23)
- 使用飞书 MCP 拉取 Wiki 对应云文档纯文本并与本文对齐
- 「文档结构」表与飞书一致（会话域并入 Management 行说明）
- 3.2.2 调用技能：Request Body、`done` 事件示例与飞书文档一致

### v2.2 (2026-03-19)
- 同步飞书 Wiki 最新表格内容
- 2.1.9 按 ID 编辑分身：补充完整 Response 结构（含 knowledge_grants）
- 2.1.10 分身软删除：新增 `DELETE /api/v1/avatars/{id}` 接口说明
- 新增 3.2 数字医生技能管理（查询技能列表、调用技能流式响应）
...
（会话与数字医生对话、知识搜索的接口见文中 **三、Management 域 — 会话管理**。）
...
**Parameters**：`skill_code`（string，路径参数）
...
`args` 内字段以各技能的 `args_schema` 为准，上表为与飞书文档一致的示例；`context` 可按业务传入会话、分身等扩展字段（不限于示例中的占位结构）。
...
SSE 事件流（每行 `data: {JSON}\n\n`）。事件类型与字段约定复用会话流式协议：`thinking_step`、`delta`、`done`、`error`。
...
data: {"type":"done","skill_run_id":"uuid","tokens_used":850,"duration_ms":560,"result":{"parts":[{"type":"text","text":"最终技能输出..."}],"citations":["对象1","对象2","对象3"]}}
data: {"type":"error","code":50002,"message":"AI 服务暂时不可用，请稍后重试"}
```
