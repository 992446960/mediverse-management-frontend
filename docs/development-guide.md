...
- `md-editor-v3` 编辑器必须绑定 `themeStore.isDark` 并复用 `src/styles/editor.css` 中的项目主题变量；禁止只引入库默认 light 样式。
- Ant Design Vue 输入控件的普通态、聚焦态、文本选区和浏览器 autofill 背景必须由全局主题层控制，新增局部输入样式时需继续使用 `--color-bg-container`、`--color-text-base`、`--color-border` 等语义变量。
...
- `pnpm verify` 会运行 `pnpm check:theme`，拦截 Vue 组件中新出现的未主题化白底、近黑文字、无效 CSS 变量任意值和全局浅色 Markdown CSS import；静态契约单测同时覆盖 Markdown/编辑器/输入控件主题入口。
