...
使用同级 Python mock 后端联调时，先在工作区根目录启动：

```bash
cd ../mediverse-management-mock-backend
...
使用 Python mock 后端跑 API 合规测试时，不读取 `.env.api-test`：
...
| 账号          | 权限       | 密码     |
| ------------- | ---------- | -------- |
| `dev001-user` | 普通用户   | `123456` |
| `dev001-dept` | 科室管理员 | `123456` |
| `dev001-org`  | 机构管理员 | `123456` |
