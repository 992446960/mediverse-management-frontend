...
- [ ] `KBSidebar/index.vue` — 整侧边栏 → 语义类/变量
- [ ] `KnowledgeCardViewer/` 系列（VersionDiffView / JsonContentPane / CardContentBody / VersionTimeline / index.vue）
- [ ] `AvatarConfig/ToolSkillSelector.vue`
- [ ] `AvatarDetailModal.vue`、`RecallSourceDetailModal.vue`、`KnowledgeCardEditor` inline
- 验证：逐个组件暗黑切换目检无白底/黑字
...
## 进度
当前阶段：**阶段 0 已完成并通过 `pnpm verify`**；下一步进入阶段 1（类别 A 暗黑可见 bug）。
