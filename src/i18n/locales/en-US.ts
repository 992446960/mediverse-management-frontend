export default {
  common: {
    all: 'All',
    user: 'User',
    login: 'Login',
    logout: 'Logout',
    copy: 'Copy',
    copied: 'Copied',
    copyFailed: 'Copy failed',
    send: 'Send',
    stop: 'Stop',
    dashboard: 'Dashboard',
    settings: 'Settings',
    theme: 'Theme',
    language: 'Language',
    profile: 'Profile',
    confirm: 'Confirm',
    cancel: 'Cancel',
    close: 'Close',
    prev: 'Previous',
    next: 'Next',
    search: 'Search',
    reset: 'Reset',
    refresh: 'Refresh',
    add: 'Add',
    edit: 'Edit',
    delete: 'Delete',
    save: 'Save',
    selectFile: 'Select file',
    loading: 'Loading...',
    noData: 'No Data',
    success: 'Success',
    error: 'Error',
    createdAt: 'Created At',
    actions: 'Actions',
    confirmDeleteTitle: 'Confirm Delete',
    confirmDeleteContent: 'This action cannot be undone. Continue?',
    confirmStatusTitle: 'Confirm',
    confirmStatusContent: "Are you sure you want to change this organization's status?",
    required: 'required',
    max50: 'max 50 characters',
    max100: 'max 100 characters',
    totalCount: 'Total {count}',
    lastUpdateTime: 'Last updated: {time}',
    detail: 'Detail',
    more: 'More',
    clearSelection: 'Clear selection',
    selectedCount: '{count} selected',
    featureInDevelopment: 'Feature In Development',
    featureInDevelopmentDesc: 'This feature is currently under development. Stay tuned.',
    back: 'Back',
    columnSettings: 'List Settings',
    listSettingsDesc: 'Configure column visibility, width dragging and fixed position',
    selectionColumn: 'Selection',
    columnName: 'Column',
    columnVisible: 'Visible',
    columnResizable: 'Resizable',
    columnFixed: 'Fixed Position',
    notFixed: 'Not Fixed',
    fixedLeft: 'Fixed Left',
    fixedRight: 'Fixed Right',
    locked: 'Locked',
    startDate: 'Start Date',
    endDate: 'End Date',
    collapse: 'Collapse',
    expand: 'Expand',
    moreFilters: 'More Filters',
    today: 'Today',
    yesterday: 'Yesterday',
    last7Days: 'Last 7 days',
    last30Days: 'Last 30 days',
    loadError: 'Load Error',
    updatedAt: 'Last Updated',
    createdBy: 'Created By',
    query: 'Query',
    minValue: 'Min',
    maxValue: 'Max',
    loginExpired: 'Login expired. Please log in again.',
    requestFailed: 'Request failed',
    downloadFallback: 'Download failed',
    repeatSubmit: 'Data is being processed. Please do not submit repeatedly.',
    resizePanelHint: 'Drag to resize width; double-click to reset default width',
    resizeDirectoryWidth: 'Drag to resize directory width',
    resizeTreeWidth: 'Drag to resize filter tree width',
    collapseDirectory: 'Collapse directories',
    collapseTree: 'Collapse filter tree',
    expandPanel: 'Expand {label}',
    expandPanelHint: 'Click to expand {label}',
    filterTree: 'Filter',
    switchToLight: 'Switch to light mode',
    switchToDark: 'Switch to dark mode',
  },
  app: {
    brandName: 'Mediverse Management',
  },
  error: {
    unauthorizedSubtitle: 'You are not authenticated. Please log in first.',
    forbiddenSubtitle: 'You are not authorized to access this page.',
    notFoundSubtitle: 'The page you visited does not exist.',
    goBack: 'Go Back',
    goToLogin: 'Go to Login',
    backHome: 'Back Home',
    http: {
      400: 'Invalid request parameters',
      401: 'Login expired. Please log in again.',
      403: 'You do not have permission to access this resource',
      404: 'The requested resource does not exist',
      405: 'Request method is not allowed',
      408: 'Request timed out. Try again later.',
      409: 'Data conflict. Refresh and try again.',
      413: 'Uploaded content is too large',
      422: 'Request parameter validation failed',
      429: 'Too many requests. Try again later.',
      500: 'Internal server error',
      502: 'Gateway error',
      503: 'Service temporarily unavailable. Try again later.',
      504: 'Gateway timeout',
      fallback: 'Request failed ({status})',
    },
  },
  avatarTest: {
    personalEmpty: 'No expert avatar found for your account',
    deptEmpty: 'No specialist avatar found for this department',
    orgEmpty: 'No general avatar found for this organization',
    configureAvatar: 'Configure Avatar',
  },
  tagsView: {
    refresh: 'Refresh',
    closeCurrent: 'Close Current',
    closeOthers: 'Close Others',
    closeLeft: 'Close Left',
    closeRight: 'Close Right',
    closeAll: 'Close All',
  },
  org: {
    title: 'Organization Management',
    name: 'Name',
    namePlaceholder: 'Enter organization name',
    code: 'Code',
    description: 'Description',
    status: 'Status',
    logo: 'Organization Logo',
    logoSizeHint: 'Recommended: 200x200px',
    logoUpload: 'Upload image',
    logoLoadFailed: 'Image failed to load, click to re-upload',
    logoUploadHint: 'JPG, PNG, WebP supported, recommended ≤ 2MB',
    logoFormatHint: 'Please select an image in JPG, PNG or WebP format',
    addOrg: 'Add Organization',
    editOrg: 'Edit Organization',
  },
  dept: {
    name: 'Department Name',
    code: 'Department Code',
    org: 'Organization',
    branch: 'Branch',
    description: 'Description',
    status: 'Status',
    title: 'Department Management',
    allOrgs: 'All Organizations',
    searchOrg: 'Search organizations',
    searchOrgOrDept: 'Search organization or department',
    searchDeptName: 'Search department name',
    namePlaceholder: 'Enter department name',
    allStatus: 'All Status',
    deptList: 'Department List',
    addDept: 'Add Department',
    editDept: 'Edit Department',
    selectOrgFirst: 'Please select an organization on the left',
    confirmStatusContent: "Are you sure you want to change this department's status?",
  },
  user: {
    username: 'Username',
    realName: 'Real Name',
    phone: 'Phone',
    email: 'Email',
    phoneInvalid: 'Please enter a valid mobile number (11 digits, starting with 1)',
    emailInvalid: 'Please enter a valid email address',
    org: 'Organization',
    dept: 'Department',
    roles: 'Roles',
    status: 'Status',
    addUser: 'Add User',
    editUser: 'Edit User',
    detailTitle: 'User Detail',
    organizationScope: 'Organization Scope',
    selectOrgDeptFirst: 'Please select an organization or department on the left',
    roleEditor: 'Role Settings',
    resetPassword: 'Reset Password',
    confirmResetPassword: 'Reset password for {name}?',
    confirmResetPasswordToDefault: 'Reset password for {name}? It will be set to default 123456',
    resetPasswordSuccessDefault: 'Password has been reset to default 123456',
    defaultPasswordCopy: 'Default password 123456 copied',
    confirmDeactivate: 'Deactivate this user?',
    confirmActivate: 'Activate this user?',
    tempPasswordTitle: 'Temporary Password',
    tempPasswordCopy: 'Copied to clipboard',
    tempPasswordTip: 'Please keep it safe. Shown only once.',
    roleSysadmin: 'System Admin',
    roleOrgAdmin: 'Org Admin',
    roleDeptAdmin: 'Dept Admin',
    roleUser: 'User',
    roleAtLeastOne: 'Please select at least one role',
    keywordPlaceholder: 'Username / Name / Phone',
    initialPasswordLabel: 'Initial password',
    passwordOptionalPlaceholder: 'Leave blank for default 123456',
    remark: 'Remark',
    usernameLength: '4-50 letters, numbers, underscores',
    createSuccessWithPassword: 'Success. Initial password: {password}',
  },
  status: {
    active: 'Active',
    inactive: 'Inactive',
    unknown: 'Unknown',
  },
  profile: {
    title: 'Profile',
    editHint: 'Edit fields on the left; right side shows profile card preview.',
    formTitle: 'Edit profile',
    previewTitle: 'Profile card preview',
    saveChanges: 'Save changes',
    changePassword: 'Change password',
    changeAvatar: 'Change avatar',
    avatarFormatHint: 'Supports JPG, PNG format, max 2MB',
    avatarSizeExceeded: 'Avatar size must not exceed 2MB',
    phonePlaceholder: 'Enter phone number',
    emailPlaceholder: 'Enter email',
    remarkPlaceholder: 'Enter personal intro or work focus...',
    saveSuccess: 'Saved successfully',
    saveFailed: 'Save failed',
    refreshSuccess: 'Refreshed',
    notLoggedIn: 'Not logged in',
  },
  menu: {
    login: 'Login',
    changePassword: 'Change Password',
    logout: 'Logout',
    dashboard: 'Dashboard',
    profile: 'Profile',
    myWorkbench: 'My Workbench',
    deptWorkbench: 'Dept Workbench',
    orgWorkbench: 'Org Workbench',
    digitalDoctor: 'Digital Doctor',
    avatarChat: 'Avatar Chat',
    knowledgeBase: 'Knowledge Base',
    systemManagement: 'System Management',
    orgManagement: 'Org Management',
    deptManagement: 'Dept Management',
    userManagement: 'User Management',
    avatarManagement: 'Avatar Management',
    apiToken: 'API Token',
    files: 'Files',
    deptFilesManagement: 'Dept Files',
    orgFilesManagement: 'Org Files',
    knowledgeCards: 'Knowledge Cards',
    avatarConfig: 'Avatar Config',
    avatarTest: 'Avatar Test',
  },
  dashboard: {
    noAvatarData: 'No avatar is available for this account. Statistics show as zero.',
    loadFailed: 'Failed to load dashboard data. Please try again later.',
    tokenBreakdown: 'Token usage by scope',
    todayToken: 'Today: ',
    totalToken: 'Total: ',
    knowledgeProgress: 'Knowledge indexing progress',
    knowledgeProgressEmpty: 'No indexed file data yet.',
    knowledgeFilesIndexed: 'Indexed knowledge files',
    knowledgeFilesRatio: '{indexed} / {total}',
    knowledgeProgressPercent: 'Indexing progress',
    percentValue: '{n}%',
    recentActivity: 'Recent activity',
    emptyActivity: 'No recent activity',
    owner: {
      personal: 'Personal avatar',
      dept: 'Department avatar',
      org: 'Organization avatar',
    },
  },
  knowledge: {
    directory: 'Directories',
    recentFiles: 'Recent Files',
    recentCards: 'Recent Cards',
    allFiles: 'All Files',
    uncategorized: 'Uncategorized',
    searchDirectory: 'Search directories',
    addDirectory: 'Add Directory',
    addDirectoryUnderParent: 'Add directory under 「{parent}」',
    addRootDirectory: 'Add Root Directory',
    addSubtitle: 'Add Sub-directory',
    renameDirectory: 'Rename directory',
    deleteDirectory: 'Delete directory',
    deleteDirectoryConfirm:
      'Delete this directory? Only empty non-default directories can be deleted.',
    directoryName: 'Directory Name',
    directoryNamePlaceholder: 'Enter directory name',
    fileName: 'File Name',
    fileType: 'Type',
    fileSize: 'Size',
    cardCount: 'Knowledge Cards',
    status: 'Status',
    publishOrg: 'Publishing Org',
    publishTime: 'Publishing Time',
    fileStatus: 'File Status',
    searchFilePlaceholder: 'Search file name',
    statusProcessing: 'Processing',
    statusDone: 'Done',
    statusFailed: 'Failed',
    retry: 'Retry',
    batchMoveFiles: 'Move files',
    batchMoveNoSelection: 'Select files first',
    batchMoveSuccess: 'Moved {count} files',
    batchDeleteFiles: 'Delete files',
    batchDeleteNoSelection: 'Select files first',
    batchDeleteFilesConfirm: 'Delete the selected {count} file(s)? This cannot be undone.',
    batchDeleteFilesSuccess: 'Deleted {count} file(s)',
    targetDirectory: 'Target directory',
    retryTaskMissing: 'This file has no retry task ID',
    retryTaskSubmitted: 'Retry task submitted',
    preview: 'Preview',
    uploadFile: 'Upload File',
    uploadCategoryMethod: 'Categorization method',
    uploadAutoCategory: 'Auto category',
    uploadManualCategory: 'Select directory',
    uploadSelectDirectoryLabel: 'Select directory',
    uploadQueueHint:
      'You can drag more files to add; category and directory apply to files not yet started.',
    uploadStart: 'Upload',
    selectDirectory: 'Select directory',
    uploadDragText: 'Click or drag files here to upload',
    uploadAcceptHint: 'Accepted formats: {accept}',
    uploadSizeHint: 'Max {size} per file, up to 20 files',
    uploadFormatInvalid: '「{name}」 format not supported. Accepted: {accept}',
    uploadSizeInvalid: '「{name}」 exceeds size limit ({size})',
    uploadMaxCountInvalid: 'Maximum {max} files',
    uploadDuplicate: 'This file is already in the upload queue',
    uploadMd5Failed: 'File verification failed. Try again.',
    uploadStatusPending: 'Pending',
    uploadStatusUploading: 'Uploading',
    uploadStatusSuccess: 'Success',
    uploadStatusFail: 'Failed',
    uploadRemove: 'Remove',
    uploadRemoveAll: 'Remove all',
    uploadSuccess: 'Upload successful',
    uploadFailed: 'Upload failed',
    uploadErrorNetwork: 'Network error. Check your connection and try again.',
    uploadErrorTimeout: 'Request timed out. Try again later.',
    uploadErrorHttp: 'Upload failed (HTTP {status})',
    pendingUploadCount: 'Pending ({count})',
    uploadingCount: 'Uploading ({count})',
    uploadCloseConfirmTitle: 'Confirm close',
    uploadCloseConfirmContent:
      '{count} file(s) uploading. They will continue in background. Close anyway?',
    uploadLeaveConfirm: 'Files are uploading. Leaving will abort uploads. Leave anyway?',
    download: 'Download',
    downloadFailed: 'Download Failed',
    noAvatarTitle: 'No Avatar Created',
    noAvatarSubTitle:
      'Please create your digital doctor avatar before managing personal knowledge base.',
    goToCreateAvatar: 'Create Avatar',
    filePreview: 'File Preview',
    parsedView: 'Parsed View',
    rawView: 'Raw View',
    unsupportedPreview: 'Preview not supported for this file type',
    loadFileFailed: 'Failed to load file',
    noPreviewUrl: 'No preview URL',
    previewNoUrlHint: 'File URL is invalid. Please try again later or contact the administrator.',
    pdfLoadFailed: 'PDF load failed: {msg}',
    pdfUnknownError: 'Unknown error',
    noParsedDoc: 'No parsed document',
    loadParsedDocFailed: 'Failed to load parsed document',
    ownerPersonal: 'Personal',
    ownerDept: 'Department',
    ownerOrg: 'Organization',
    ownerAvatar: 'Avatar',
    downloadOriginal: 'Download Original',
    fileContentPreview: 'File Content Preview',
    statusBadgeParsed: 'Parsed',
    statusBadgeFailed: 'Parse Failed',
    missingFileId: 'Missing file ID',
    missingFileUrl: 'Missing file URL',
    unsupportedFileType: 'This file type is not supported for preview',
    downloadFile: 'Download file',
    enterPreviewFromList: 'Please open preview from the file list',
    noDeptPermission: 'No department permission',
    noOrgPermission: 'No organization permission',
    pleaseLogin: 'Please login first',
    previewZoomReset: 'Reset zoom',
    previewToc: 'Outline',
    previewScrollPrevScreen: 'Previous screen',
    previewScrollNextScreen: 'Next screen',
    previewExcelSheetHint: 'Switch sheets using the tab bar at the bottom of the grid.',
    previewPptxHint: 'Zoom to browse slides; complex animations may differ from desktop Office.',
    previewDocLegacy:
      'Legacy .doc files cannot be previewed in the browser. Use Download Original to open in Word.',
    card: {
      title: 'Knowledge Cards',
      create: 'New Knowledge Card',
      editTitle: 'Edit Knowledge Card',
      createTitle: 'New Knowledge Card',
      titleLabel: 'Title',
      titlePlaceholder: 'Enter knowledge card title',
      titleRequired: 'Please enter title',
      typeLabel: 'Type',
      typePlaceholder: 'Select type',
      typeRequired: 'Please select type',
      tagsLabel: 'Tags',
      addTag: 'Add Tag',
      contentLabel: 'Content',
      contentPlaceholder: 'Enter knowledge card content...',
      contentRequired: 'Please enter content',
      saveSuccess: 'Update submitted successfully, waiting for update',
      createSuccess: 'Create submitted successfully, waiting for creation',
      searchLabel: 'Search',
      searchPlaceholder: 'Search title or content',
      onlineStatus: 'Status',
      auditStatus: 'Audit Status',
      typeRule: 'Rule',
      typeScale: 'Scale',
      typeRiskPoint: 'Risk Control Point',
      typePathwayClause: 'Pathway Clause',
      typeMelodyElement: 'Melody Element',
      typeScoreElement: 'Score Element',
      typeDiseaseOverview: 'Disease Overview',
      typeEvidence: 'Evidence',
      typeDoctorVisit: 'Doctor Visit',
      typeDoctorTrajectory: 'Doctor Trajectory',
      typeDoctorSummary: 'Doctor Summary',
      columnTitle: 'Title',
      columnType: 'Type',
      columnStatus: 'Status',
      columnAuditStatus: 'Audit Status',
      columnReferenceCount: 'References',
      columnUpdatedAt: 'Updated Time',
      online: 'Go Online',
      offline: 'Go Offline',
      creating: 'Creating',
      updating: 'Updating',
      onlineStatusOnline: 'Online',
      onlineStatusOffline: 'Offline',
      onlineStatusCreating: 'Creating',
      onlineStatusUpdating: 'Updating',
      auditPending: 'Pending',
      auditApproved: 'Approved',
      auditRejected: 'Rejected',
      onlineToggle: 'Toggle Online',
      confirmToggleOnline: 'Are you sure to go online this card?',
      confirmToggleOffline: 'Are you sure to go offline this card?',
      confirmDelete: 'Are you sure to delete this card? This action cannot be undone.',
      fetchFailed: 'Failed to fetch list',
      deleteSuccess: 'Deleted successfully',
      deleteFailed: 'Delete failed',
      onlineSuccess: 'Online',
      offlineSuccess: 'Offline',
      offlineNotePlaceholder: 'Enter offline reason (optional)',
      onlineBlockedByAudit: 'Only approved knowledge cards can go online',
      creatingBlocked: 'The knowledge card is being processed. Please refresh later.',
      detailTitle: 'Knowledge Card Detail',
      fetchDetailFailed: 'Failed to fetch details',
      rollbackSuccess: 'Successfully rolled back to previous version {version}',
      rollbackFailed: 'Rollback failed',
      rollbackConfirmTitle: 'Confirm Rollback',
      rollbackConfirmContent:
        'Are you sure to roll back the current version to previous version {version}?',
      rollbackReasonPlaceholder: 'Enter rollback reason (optional)',
      rollbackInvalidVersion:
        'Only rollback from the current version to the previous version is supported.',
      currentVersion: 'Current',
      operator: 'Operator',
      tabContent: 'Content',
      tabVersions: 'Version History',
      tabDiff: 'Version Diff',
      tabInfo: 'Metadata',
      sourceFileFilter: 'Source File',
      sourceFilePlaceholder: 'Select or search files',
      sourceFileLabel: 'Associated Files',
      tagFilterPlaceholder: 'Enter tag to filter',
      compare: 'Compare',
      diffUnified: 'Unified',
      diffSplit: 'Split',
      diffFailed: 'Failed to load version diff',
      diffNoData: 'Select two different versions to compare',
      diffFromLabel: 'Old version',
      diffToLabel: 'New version',
      sourceFile: 'Associated files',
      attachmentCount: '{count} attachments',
      sourceFileNotFoundInWorkspace:
        'File not found in the workspace file list; preview unavailable',
      sourceFilePreviewUnsupported: 'Preview from here is not supported for avatar workspace files',
      noSourceFile: 'No associated files',
      cardId: 'Card ID',
      referenceCount: 'References',
      times: 'times',
      notFound: 'Knowledge card not found',
      extractedCards: 'Extracted Cards ({count})',
      source: 'Source:',
      noCards: 'No knowledge cards',
      rollback: 'Rollback',
      changeSummaryLabel: 'Change Summary',
      changeSummaryPlaceholder: 'Briefly describe the changes',
      jsonPane: 'JSON',
      markdownPane: 'Markdown',
      copyJson: 'Copy JSON',
      jsonEmpty: 'No JSON data available',
      markdownEmpty: 'No Markdown content available',
      tagMaxLimit: 'Maximum 10 tags allowed',
      auditApprove: 'Approve',
      auditReject: 'Reject',
      auditApproveConfirm: 'Are you sure to approve this knowledge card?',
      auditRejectConfirm: 'Please enter the rejection reason:',
      auditRejectReasonPlaceholder: 'Enter rejection reason (required)',
      auditRejectReasonRequired: 'Rejection reason is required',
      auditSuccess: 'Audit operation succeeded',
      auditFailed: 'Audit operation failed',
      auditRejectReason: 'Rejection Reason',
      batchSelectedCount: '{count} selected',
      batchNoSelection: 'Select knowledge cards first',
      batchAuditApprove: 'Approve selected',
      batchAuditReject: 'Reject selected',
      batchAuditApproveConfirm: 'Approve the selected {count} knowledge card(s)?',
      batchAuditRejectConfirm:
        'Enter a rejection reason. It will apply to the selected {count} knowledge card(s):',
      batchAuditApproveSuccess: 'Approved {count} selected knowledge card(s)',
      batchAuditRejectSuccess: 'Rejected {count} selected knowledge card(s)',
      batchOnline: 'Online selected',
      batchOffline: 'Offline selected',
      batchOnlineSuccess: 'Published {count} knowledge card(s)',
      batchOfflineSuccess: 'Unpublished {count} knowledge card(s)',
      batchDelete: 'Delete selected',
      batchDeleteConfirm:
        'Delete the selected {count} knowledge card(s)? This action cannot be undone.',
      batchDeleteSuccess: 'Deleted {count} knowledge card(s)',
    },
    recall: {
      title: 'Knowledge Card Recall Test',
      subtitle: 'Validate semantic recall accuracy and AI answer quality',
      entry: 'Recall Test',
      queryLabel: 'Test query',
      queryPlaceholder:
        'Enter a test question, e.g. How should knowledge base permissions be managed?',
      paramsTitle: 'Parameters',
      cardTypeLabel: 'Knowledge card type',
      allTypes: 'All types',
      topKLabel: 'Top-K',
      execute: 'Run Test',
      reset: 'Reset',
      loading: 'Retrieving knowledge cards...',
      resultTitle: 'Test Results',
      emptyResult:
        'Enter a query and run the test to view the final answer and retrieved knowledge cards.',
      finalAnswer: 'Final Answer',
      retrievedSources: 'Retrieved Knowledge Cards',
      sourceCount: '{count} retrieved',
      queryTime: 'Latency: {time}',
      confidence: 'Confidence: {value}',
      noAnswer: 'No final answer',
      noSources: 'No retrieved knowledge cards',
      viewCardDetail: 'View knowledge card detail',
      noCardContent: 'No knowledge card content',
      previewContentFallbackHint:
        'This knowledge card has no Markdown content. The recall preview is shown instead.',
      recallPerformance: 'Recall Performance',
      recallScore: 'Recall Score',
      relatedFiles: 'Related Files',
      noRelatedFiles: 'No related files',
      detailLoadFailed: 'Failed to load knowledge card detail',
      queryRequired: 'Enter a test query',
      ownerMissing: 'Current workspace is missing a knowledge base ID',
      cardTypeRequired: 'Select at least one knowledge card type',
      historyEntry: 'Recall History',
      historyTitle: 'Recall History',
      historySubtitle: 'Review previous recall sessions, answers, and retrieved knowledge cards.',
      statusLabel: 'Status',
      statusSuccess: 'Success',
      statusFailed: 'Failed',
      statusTimeout: 'Timeout',
      statusError: 'Error',
      queryColumn: 'Query',
      cardCount: 'Retrieved',
      latency: 'Latency',
    },
  },
  knowledgeSearch: {
    title: 'Knowledge Base Search',
    subtitle: 'AI-powered Q&A assistant across the knowledge base',
    searchPlaceholder: 'Search all knowledge...',
    searchButton: 'Search',
    recentSearches: 'Recent Searches',
    newSearch: 'New Search',
    noTitle: 'Untitled Session',
    deleteSession: 'Delete Session',
    followUpPlaceholder: 'Ask a follow-up question...',
    aiDisclaimer:
      'AI-generated content is for reference only. Please consult a doctor for diagnosis.',
    citationLabel: 'Sources',
    knowledgeCardHitsTitle: 'Knowledge cards',
    matchedFilesLabel: 'Matched files',
    openFilePreview: 'Open file preview in a new tab',
    viewOriginal: 'View Original',
    startNewSearch: 'Start a new search...',
    groupToday: 'Today',
    groupYesterday: 'Yesterday',
    groupWeek: 'Past 7 Days',
    groupEarlier: 'Earlier',
    recentHistoryLoadFailed: 'Failed to load recent searches',
    recentHistoryDeleteFailed: 'Failed to delete recent search',
    clearRecentSearches: 'Clear all',
    clearRecentSearchesTitle: 'Clear recent searches',
    clearRecentSearchesConfirm: 'Clear all recent search history? This cannot be undone.',
    clearRecentSearchesSuccess: 'Recent searches cleared',
    clearRecentSearchesFailed: 'Failed to clear recent searches',
    homeRecentFilesSection: 'Latest files',
    homeRecentCardsSection: 'Latest knowledge cards',
    homeFilesPersonal: 'Personal — latest files',
    homeFilesDept: 'Department — latest files',
    homeFilesOrg: 'Organization — latest files',
    homeCardsPersonal: 'Personal — latest knowledge cards',
    homeCardsDept: 'Department — latest knowledge cards',
    homeCardsOrg: 'Organization — latest knowledge cards',
    viewAll: 'View all',
    homeNoWorkbenchTitle: 'No workbench access',
    homeNoWorkbenchSubtitle:
      'Your account has no avatar workbench enabled, so recent files and knowledge cards cannot be shown.',
    homeColumnEmpty: 'No data',
    homeColumnLoadFailed: 'Failed to load',
    homeRelativeUpdated: 'Updated {time}',
  },
  chat: {
    newSession: 'New Conversation',
    startNewConversation: 'Start a new conversation',
    selectAvatarHint: 'Select a digital doctor to start consultation',
    noSessions: 'No conversations yet',
    noSessionsHint:
      'No conversation history yet. Please select a digital doctor on the right to start consultation.',
    emptyMessages: 'Start a conversation',
    emptyMessagesHint: 'Your intelligent medical assistant, ready to provide professional support',
    tryAsking: 'You can try asking me:',
    renamePlaceholder: 'Enter new title',
    renameSessionTitle: 'Rename Conversation',
    renameSessionName: 'Conversation Title',
    renameRequired: 'Please enter a conversation title',
    rename: 'Rename',
    delete: 'Delete',
    groupToday: 'Today',
    groupYesterday: 'Yesterday',
    groupWeek: 'This Week',
    groupEarlier: 'Earlier',
    untitledSession: 'Untitled Session',
    inputPlaceholder: 'Type a message, Shift+Enter for newline...',
    thinkingDeep: 'Deep Think',
    thinkingBalance: 'Balanced',
    thinkingFast: 'Fast',
    webSearchOn: 'Web Search: On',
    webSearchOff: 'Web Search: Off',
    attachFile: 'Attach file',
    attachmentDuplicate: '"{name}" already exists, skipped',
    attachmentMaxCount: 'Maximum {max} attachments',
    attachmentFormatInvalid: '"{name}" format not supported',
    attachmentSizeExceeded: '"{name}" exceeds size limit',
    attachmentLegacyDocHint:
      'Legacy Word (.doc) cannot be previewed in the page. The file was downloaded; open it locally with Word or another app.',
    attachmentFallbackDownload:
      'This file type cannot be previewed in the page. It was downloaded; open it on your computer.',
    rateConversation: 'Rate',
    rated: 'Rated',
    prompt1: 'Help me analyze this case',
    prompt2: 'Recommend latest hypertension guidelines',
    prompt3: 'How to handle doctor-patient communication?',
    quotaExhausted: "This avatar's experience quota has been exhausted",
    viewSource: 'Sources',
    skillPanel: 'Skills',
    skillPanelExpand: 'Open skills panel',
    skillPanelCollapse: 'Close skills panel',
    skillPanelTitle: 'Digital Doctor Skills',
    skillExecute: 'Execute',
    skillBack: 'Back',
    skillExecutionResult: 'Execution Result',
    skillNeedInput: 'Please enter content',
    skillNeedFile: 'Please upload an attachment',
    skillNeedLogin: 'Please sign in to use skills',
    skillMissingField: 'Missing required argument: {field}',
    createSessionBackendInvalid:
      'Failed to create conversation: backend did not return a valid session.id',
    createSessionFrontendInvalid:
      'Failed to create conversation: frontend did not receive a valid session.id',
    skillTagHypertensionGuideline: 'Hypertension Guidelines',
    skillTagDiabetesDiet: 'Diabetes Diet',
    skillNoRecords: 'No call records yet',
    skillCallArgs: 'Args',
    skillCallResult: 'Result',
    skillCallDuration: 'Duration',
    toolCallsSection: 'Tools used',
    toolCallsSectionExpand: 'Expand tools used',
    toolCallsSectionCollapse: 'Collapse tools used',
    toolCallInvokedTimes: '{n} invocations',
    toolCallFallbackTitle: 'Tool',
    message: 'Message',
    scrollToBottom: 'Scroll to bottom',
    thinkingProcessTitle: 'Thinking Process',
    thinkingProcessStepDetailExpand: 'Expand step details',
    thinkingProcessStepDetailCollapse: 'Collapse step details',
    testSessionTitle: 'Avatar test',
  },
  theme: {
    light: 'Light Mode',
    dark: 'Dark Mode',
    auto: 'System Default',
    presetSelector: 'Color Theme',
  },
  avatar: {
    name: 'Avatar Name',
    type: 'Type',
    typeGeneral: 'General',
    typeSpecialist: 'Specialist',
    typeExpert: 'Expert',
    typeGeneralAvatar: 'General Avatar',
    typeSpecialistAvatar: 'Specialist Avatar',
    typeExpertAvatar: 'Expert Avatar',
    selectOrg: 'Select Organization',
    selectDept: 'Select Department',
    searchName: 'Search avatar name',
    noBio: 'No description',
    noAvatars: 'No avatars',
    scope: 'Organization',
    org: 'Organization',
    dept: 'Department',
    bindUser: 'User',
    avatar: 'Avatar',
    avatarUrl: 'Avatar URL',
    bio: 'Bio',
    tags: 'Tags',
    greeting: 'Greeting',
    style: 'Communication Style',
    styleCustom: 'Custom Style',
    addAvatar: 'Add Avatar',
    editAvatar: 'Edit Avatar',
    configSaveMissingAvatarId: 'Cannot save: avatar id is missing. Please refresh and try again.',
    detailTitle: 'Avatar details',
    testChatTitle: '{name} chat test',
    testAvatarIdHint: 'Test avatar ID: {id}',
    knowledgeGrants: 'Knowledge grants',
    grantScope: 'Scope',
    scopeType: 'Scope type',
    grantedBy: 'Granted by',
    grantedAt: 'Granted at',
    confirmStatusContent: "Are you sure you want to change this avatar's status?",
    scopeDisplay: 'Organization: {scope}',
    overview: 'Avatar overview',
    changeAvatar: 'Change avatar',
    advanced: {
      title: 'Advanced Capabilities',
      tools: 'Tools',
      skills: 'Skills',
      engine: 'Reasoning Engine',
      model: 'Model',
      addTool: 'Add tool',
      addSkill: 'Add skill',
      selectTool: 'Select tools',
      selectSkill: 'Select skills',
      searchTool: 'Search tool name, capability, or keyword',
      searchSkill: 'Search skill name, capability, or keyword',
      confirmSelected: 'Confirm ({count} selected)',
      emptySelector: 'No options',
      enginePlaceholder: 'Select reasoning engine',
      providerPlaceholder: 'Select model family',
      modelPlaceholder: 'Select model',
    },
    wizard: {
      selectType: 'Select Type',
      bindScope: 'Binding Scope',
      basicInfo: 'Basic Info',
      confirm: 'Confirm',
      stepProgress: 'Step {current} of {total}',
      avatarPreview: 'Avatar Preview',
      confirmSectionTitle: 'Please confirm the following information',
      scopeHint: 'Select organization first, then department (and user for expert avatar).',
      scopePreviewTitle: 'Current binding scope preview',
      placeholderOrg: 'Please select organization',
      placeholderDept: 'Please select department',
      placeholderUser: 'Please select expert user',
      selectUser: 'Select user',
      expertOnlyRequired: '(required for expert avatar only)',
      descGeneral: 'General digital doctor for the whole organization',
      descGeneralDetail: 'Basic triage and general Q&A across the entire hospital',
      descSpecialist: 'Specialist digital doctor for a department',
      descSpecialistDetail:
        'Professional medical advice and follow-up for specific clinical departments',
      descExpert: 'Expert digital twin for an individual',
      descExpertDetail: "Personalized avatar built on a specific expert's knowledge system",
      placeholderName: 'Enter avatar name',
      placeholderBio: 'Enter avatar bio...',
      placeholderGreeting: 'Hello, I am your digital doctor assistant...',
      avatarUpload: 'Upload',
      removeAvatar: 'Remove avatar',
      avatarSizeHint: 'Recommended: 200x200px',
      avatarFormatHint: 'JPG, PNG, GIF',
      addTag: 'Add tag',
      tagPlaceholder: 'Enter tag...',
      tagPlaceholderAdd: 'Add tag',
      styleFormal: 'Formal & Professional',
      styleFriendly: 'Friendly & Kind',
      styleConcise: 'Concise & Efficient',
      styleDetailed: 'Detailed & Comprehensive',
      styleCustom: 'Custom',
      styleFormalDesc: 'Authoritative and reliable',
      styleFriendlyDesc: 'Warm and easy to talk to',
      styleConciseDesc: 'Brief and focused',
      styleDetailedDesc: 'Thorough and informative',
      styleCustomDesc: 'Custom tone and style',
      afterCreateHint: 'You can continue editing advanced capability settings after creation',
      identityTitle: 'Avatar identity',
      conversationContent: 'Conversation strategy and content',
      pendingCreate: 'Pending',
      confirmCreate: 'Create',
      createSuccess: 'Avatar created successfully',
      pageTitlePersonal: 'My Digital Avatar Configuration',
      pageTitleDept: 'Dept Digital Doctor Configuration',
      pageTitleOrg: 'Org Digital Doctor Configuration',
      stats: {
        title: 'Digital Avatar Service Statistics',
        totalSessions: 'Total Sessions',
        todaySessions: 'Today Sessions',
        todayTokensUsed: 'Today Tokens Used',
        totalTokensUsed: 'Total Tokens Used',
        totalReferences: 'Knowledge Base References',
        knowledgeProgress: 'Knowledge indexing progress',
        knowledgeProgressFiles: '{indexed} / {total} files indexed',
      },
      quickGuide: {
        title: 'Quick Action Guide',
        content:
          'After completing the configuration, you can directly enter the dialogue test in the console for real-time debugging.',
        contentBefore: 'After completing the configuration, you can directly enter the ',
        contentLink: 'dialogue test',
        contentAfter: ' in the console for real-time debugging.',
        testNow: 'Test Now',
      },
      config: {
        basicInfo: 'Basic Info Configuration',
        dialogStrategy: 'Dialogue Strategy',
        placeholderName:
          'Please enter the name of the digital avatar, e.g., Clinical Assistant - Xiaolan',
        placeholderBio:
          'Please briefly describe the background and responsibilities of the digital avatar',
        placeholderGreeting: 'Hello! I am your digital clinical assistant, how can I help you?',
        avatarFormatHint: 'Supports JPG, PNG formats, recommended size 512x512px',
      },
    },
  },
  auth: {
    pleaseEnterUsernameAndPassword: 'Please enter username and password',
    loginSuccess: 'Login successful',
    rememberPassword: 'Remember password',
    username: 'Username',
    password: 'Password',
    usernamePlaceholder: 'Please enter username',
    passwordPlaceholder: 'Please enter password',
    loginButton: 'Log in',
    footer: '© 2026 Mediverse Management · Healthcare Digital Platform',
    toggleThemeAria: 'Toggle light/dark mode',
    changePasswordTitle: 'Change Password',
    changePasswordSubtitle: 'For your account security, please change your initial password',
    oldPassword: 'Old Password',
    newPassword: 'New Password',
    confirmPassword: 'Confirm New Password',
    oldPasswordPlaceholder: 'Please enter old password',
    newPasswordPlaceholder: 'Please enter new password',
    confirmPasswordPlaceholder: 'Please enter new password again',
    confirmChangeButton: 'Confirm Change',
    passwordMismatch: 'The two passwords do not match',
    passwordMinLength: 'New password must be at least 8 characters with letters and numbers',
    changePasswordSuccess: 'Password changed successfully. Please log in again.',
    oldPasswordRequired: 'Please enter old password',
    newPasswordRequired: 'Please enter new password',
    confirmPasswordRequired: 'Please enter new password again',
    changePasswordSkip: 'Modify later',
    changePasswordSkipHint: 'Skip for now, you can change password in settings later',
  },
  rating: {
    title: 'Rate This Consultation',
    accuracy: 'Diagnostic Accuracy',
    completion: 'Consultation Completeness',
    feedback: 'Additional Feedback (Optional)',
    feedbackPlaceholder: 'Please enter your suggestions...',
    submitSuccess: 'Thank you for your feedback',
    submitFailed: 'Submission failed, please try again',
    reSubmitHint: 'You have already rated. Submitting again will add a new rating.',
  },
  apiToken: {
    title: 'API Token Management',
    name: 'Token Name',
    namePlaceholder: 'Enter token name',
    token: 'Token',
    org: 'Organization',
    status: 'Status',
    lastUsedAt: 'Last Used At',
    addToken: 'Add Token',
    editToken: 'Edit Token',
    createSuccess: 'Token created successfully',
    updateSuccess: 'Token updated successfully',
    copy: 'Copy',
    copySuccess: 'Token copied to clipboard',
    copyFailed: 'Copy failed',
    confirmDelete: 'Are you sure you want to delete this token?',
    confirmStatus: 'Are you sure you want to change this token status?',
    showToken: 'Show Token',
    hideToken: 'Hide Token',
    tokenVisibleTip:
      'Please store your token securely. The full token is shown only once after creation.',
  },
}
